...
pub fn read_mask(file: &std::path::PathBuf) -> MaskShift { 
    ...
    info!("loaded {} mask regions on {} chromosomes", num_mask, ret.keys().len());
    return ret;
}
...
pub fn read_genome(file: &std::path::PathBuf, mask: &Option<MaskShift>) -> GenomeShift{
    ...
    */
    ...
    let mut shift: HashMap<String, u64> = HashMap::new();
        ...
        for line in lines {
            if let Ok(cur_line) = line {
                ...
                let masked_bases = match mask {
                    Some(m) => if m.contains_key(&chrom) {m[&chrom].cov()} else {0},
                    None => 0
                };
                let size = collection[1].parse::<u64>().unwrap() - masked_bases;
                
                shift.insert(chrom.clone(), cur_start);
                load.push(Iv{start: cur_start, stop: cur_start + size, val: size});
                cur_start += size;
    ...
    load.sort();
    return GenomeShift{chrom: Lapper::new(load), shift: shift, span: cur_start};
}

pub fn read_bed(file: &std::path::PathBuf, genome: &GenomeShift, mask: &Option<MaskShift>) -> Lapper<u64, u64> {
    /*
        ...
        for line in lines {
            if let Ok(cur_line) = line {
                ...
                if genome.shift.contains_key(&chrom) {
                    ...
                    let skip = match mask {
                        Some(m) => if m.contains_key(&chrom) {m[&chrom].find(m_start, m_stop).count() != 0} else {false},
                        None => false
                    };
                    if skip {
                        num_masked += 1;
                    ...
                    // mask will r_shift
                    ret.push(Iv{start: m_start + r_shift - l_shift,
